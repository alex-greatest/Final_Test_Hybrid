...
/// <summary>
/// Тестовый шаг проверки роста температуры контура отопления.
/// </summary>
...
{
    private const string BlockPath = "DB_VI.CH.Check_Flow_Temperature_Rise";
    private const string StartTag = "ns=3;s=\"DB_VI\".\"CH\".\"Check_Flow_Temperature_Rise\".\"Start\"";
    private const string EndTag = "ns=3;s=\"DB_VI\".\"CH\".\"Check_Flow_Temperature_Rise\".\"End\"";
    private const string ErrorTag = "ns=3;s=\"DB_VI\".\"CH\".\"Check_Flow_Temperature_Rise\".\"Error\"";

    private const string FlwTempHotTag = "ns=3;s=\"DB_Parameter\".\"CH\".\"Flw_Temp_Hot\"";
    private const string FlowNtcTempRiseTag = "ns=3;s=\"DB_Parameter\".\"CH\".\"Flow_NTC_Temp_Rise\"";

    private const string FlwTempRiseMinRecipe = "ns=3;s=\"DB_Recipe\".\"CH\".\"FlwTempRise\".\"Min\"";
    private const string FlwTempRiseMaxRecipe = "ns=3;s=\"DB_Recipe\".\"CH\".\"FlwTempRise\".\"Max\"";

    public string Id => "ch-check-flow-temperature-rise";
    public string Name => "CH/Check_Flow_Temperature_Rise";
    public string Description => "Контур Отопления. Проверка роста температуры";
    ...
    /// <summary>
    /// Выполняет шаг проверки роста температуры контура отопления.
    /// </summary>
    ...
    {
        logger.LogInformation("Запуск проверки роста температуры контура отопления");
        testResultsService.Remove("CH_Flw_Temp_Hot");
        testResultsService.Remove("CH_Flow_NTC_Temp_Rise");
    ...
    private async Task<TestStepResult> HandleCompletionAsync(TestStepContext context, bool isSuccess, CancellationToken ct)
    {
        ...
        var msg = TemperatureRiseResultMessageFormatter.Format(flwTempHot, flowNtcTempRise);
        ...
        if (isSuccess)
        {
            ...
            logger.LogInformation("Проверка роста температуры завершена успешно");
