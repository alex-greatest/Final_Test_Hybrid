...
/// <summary>
/// OLD-версия шага минимального нагрева.
...
/// </remarks>
public class ChStartMinHeatoutStepOld(
    IOptions<DiagnosticSettings> settings,
    DualLogger<ChStartMinHeatoutStepOld> logger) : IHasPlcBlockPath, IRequiresPlcSubscriptions, IProvideLimits
{
    ...
    {
        logger.LogInformation("Запуск шага минимального нагрева контура отопления");
    ...
    /// <summary>
    /// Обрабатывает Ready_1: переключает котёл в режим минимального нагрева.
    /// </summary>
    private async Task<TestStepResult> HandleReady1Async(TestStepContext context, CancellationToken ct)
    {
        logger.LogInformation("Ready_1 получен, переключаем котёл в режим минимального нагрева");

        var address = (ushort)(RegisterOperationMode - _settings.BaseAddressOffset);
        var writeResult = await context.DiagWriter.WriteUInt16Async(address, MinHeatingMode, ct);
        ...
        context.ReportProgress("Команда минимального нагрева отправлена успешно");
        logger.LogInformation("Режим минимального нагрева (3) записан успешно");
