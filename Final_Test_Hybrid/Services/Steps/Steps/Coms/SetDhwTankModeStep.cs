    ...
    DualLogger<SetDhwTankModeStep> logger) : ITestStep, INonSkippable
{
    ...
    private async Task<TestStepResult> SetTankModeAsync(TestStepContext context, CancellationToken ct)
    {
        ...
        var writeResult = await context.PacedDiagWriter.WriteUInt16Async(modbusAddress, TankModeValue, ct);
    ...
    {
        var msg = ComsStepFailureHelper.BuildWriteMessage(result, $"записи в регистр {RegisterTankModeDoc}", $"Ошибка записи в регистр {RegisterTankModeDoc}. {result.Error ?? "Неизвестная ошибка"}");
        logger.LogError(msg);
        return TestStepResult.Fail(msg);
    }
    ...
    {
        var msg = ComsStepFailureHelper.BuildReadMessage(result, $"чтении регистра {RegisterTankModeDoc}", $"Ошибка чтения регистра {RegisterTankModeDoc}. {result.Error ?? "Неизвестная ошибка"}");
        logger.LogError(msg);
        return TestStepResult.Fail(msg);
    }
