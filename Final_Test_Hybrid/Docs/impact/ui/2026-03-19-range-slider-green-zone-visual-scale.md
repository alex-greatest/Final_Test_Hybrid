...
- Подписи шкалы больше не зависят от правила `каждая вторая риска` и теперь строятся только из ключевых точек:
  ...
  - `GreenEnd`;
  - `Max`.
- Для двух дополнительных внутренних насечек зелёной зоны добавлены постоянные подписи:
  - обе внутренние точки всегда отображаются;
  - чтобы не получать повторы вида `0.80 / 0.80 / 0.80`, точность теперь рассчитывается по полному набору подписей шкалы, включая обе внутренние точки.
...
- Для одновременно показанных слайдеров точность отображения синхронизирована:
  - `RangeSliderDisplay` считает требуемую точность для каждого активного слайдера по полному набору отображаемых подписей шкалы;
  - всем активным `RangeSlider` передаётся общий максимум, чтобы экран не смешивал `1` и `2` знака после запятой в одном блоке.
- Базовая точность отображения повышена до `2` знаков после запятой:
  - это даёт стабильный вид `xx.xx` по умолчанию и для подписей шкалы, и для пузыря текущего значения;
  - повышение выше `2` остаётся только как защитный fallback для очень плотных диапазонов.
