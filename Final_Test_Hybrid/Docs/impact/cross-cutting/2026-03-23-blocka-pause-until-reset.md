...
- UI message для этой ветки добавлен отдельно: `Блокировка А. Остановите тест`.
...
4. `MessageService` показывает `Блокировка А. Остановите тест`, но приоритет `Нет связи с PLC` и `Нет автомата` оставлен выше.
