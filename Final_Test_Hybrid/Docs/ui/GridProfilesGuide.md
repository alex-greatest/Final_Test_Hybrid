...
### `grid-unified-host` + `grid-unified` + `grid-wide-editor-host` + `grid-wide-editor`
...
## Wide Editor: ключевые правила

Источник: `wwwroot/css/app.css`.

1. Профиль используется только как opt-in модификатор поверх `grid-unified`, а не как отдельная замена unified-стиля.
2. Включает `AllowColumnResize` для рабочих колонок и фиксирует служебные колонки (`select`, `actions`) через `Resizable="false"`.
3. Action-column закрепляется справа через `Frozen="true"` и `FrozenPosition="Right"`.
...
4. Колонки можно менять по ширине вручную, но дефолтная геометрия остаётся компактной и локально настроенной в `TestSequenseGrid`.
5. Колонка `Результаты` по умолчанию шире колонки `Пределы`, потому что `Result` считается основным runtime-текстом, а `Range` — вспомогательным контрактом.
6. Если для `main-grid-legacy` включён resize, host и сам `RadzenDataGrid` должны оставаться растянутыми на ширину flex-контейнера (`width:100%`, `min-width:0`, для грида также `flex:1`), иначе внутренний table-layout может схлопнуться до content-width.
7. Для стабильного первого drag-resize `TestSequenseGrid` синхронизирует фактические ширины header/col в inline-style после render через локальный JS helper; это фиксирует стартовый baseline для `Radzen` resize без переноса `main-grid-legacy` на другой table-layout.
8. Поскольку `main-grid-legacy` скрывает горизонтальный scroll, локальный JS helper дополнительно ограничивает расширение активной колонки видимой шириной грида, чтобы resizer не уходил за правую границу.
9. Не расширять этот профиль для других экранов.
