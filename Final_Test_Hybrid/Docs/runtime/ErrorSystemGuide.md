...
<FloatingErrorPanel />
```
- Локальная панель ошибки из `BoilerInfo` показывается поверх главного экрана при ошибке теста.
- По умолчанию панель стартует в правой верхней зоне экрана, а не в нижнем правом углу, чтобы не перекрывать нижний сектор `TestSequenseGrid`.
- Перетаскивание разрешено по всей поверхности панели; drag не ограничен только заголовком.
- Drag панели работает через pointer-events, а не через mouse-only lifecycle.
- Для touch-сценариев drag-зона использует `touch-action: none`, чтобы gesture браузера не перехватывал перенос панели.
- Любой `pointerup` и `pointercancel` обязан завершать drag-state; залипший перенос после touch-срыва недопустим.
...
- Drag бейджа и окна активных ошибок использует общий pointer-aware helper `wwwroot/js/floating-panel.js`; отдельные mouse/touch реализации не допускаются.
- Обычный tap/click по бейджу открывает или закрывает окно активных ошибок, а реальный drag не должен триггерить toggle.
- Для touch-сценариев сам бейдж использует `touch-action: none`, а helper обязан очищать drag-state на `pointerup` и `pointercancel`, чтобы после touch не оставалось движения за курсором.
