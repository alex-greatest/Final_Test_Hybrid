...
- Изолировать manual overlap backend-gate’ом для `ConnectionTest` и всех backend write-path’ов `HandProgram`, не трогая глобально `IModbusDispatcher`.
...
- Добавить singleton `ManualRuntimeGate` и зарегистрировать его в [`StepsServiceExtensions.cs`](/D:/projects/Final_Test_Hybrid/Final_Test_Hybrid/Services/DependencyInjection/StepsServiceExtensions.cs) рядом с runtime singleton’ами.
- `ManualRuntimeGate` должен агрегировать `ExecutionActivityTracker`, `PlcResetCoordinator`, `RuntimeTerminalState`, `IErrorCoordinator`; не оборачивать `IModbusDispatcher.StartAsync/StopAsync`.
- В [`ConnectionTestPanel.razor`](/D:/projects/Final_Test_Hybrid/Final_Test_Hybrid/Components/Overview/ConnectionTestPanel.razor) deny должен срабатывать до `ManualSessionState.EnterConnectionTest()` и до `Dispatcher.StartAsync()`, а также в `ReadRegisterAsync`, `WriteRegisterAsync`, `SetBoilerModeAsync`.
...
- Backend gate добавить в write-path’ы:
  - [`InteractiveLampOutput.razor`](/D:/projects/Final_Test_Hybrid/Final_Test_Hybrid/Components/Overview/InteractiveLampOutput.razor)
  - [`GasScheme.razor`](/D:/projects/Final_Test_Hybrid/Final_Test_Hybrid/Components/Schemes/GasScheme.razor)
  - [`HeatingScheme.razor`](/D:/projects/Final_Test_Hybrid/Final_Test_Hybrid/Components/Schemes/HeatingScheme.razor)
  - [`HotWaterScheme.razor`](/D:/projects/Final_Test_Hybrid/Final_Test_Hybrid/Components/Schemes/HotWaterScheme.razor)
- `HandProgramDialog` должен реагировать на `ManualRuntimeGate.OnChanged`, а не только на `PreExecution.OnStateChanged`.
- [`IoEditorDialog.razor.cs`](/D:/projects/Final_Test_Hybrid/Final_Test_Hybrid/Components/Engineer/Modals/IoEditorDialog.razor.cs) явно зафиксировать как residual manual write risk вне текущего пакета; не включать молча.
...
- Новый внутренний backend gate: `ManualRuntimeGate`.
- Публичные UI/API сценарии не меняются; меняется внутренняя семантика safety-check’ов, interrupt ownership и manual/runtime gating.
...
- Текущее `no new incident` удалить; зафиксировать confirmed failure modes: stale-cache false-finish/false-cleanup, `WaitForFalseAsync` false-success, manual runtime overlap/shared dispatcher ownership.
...
- Manual gate: deny до session/dispatcher start; deny action в уже открытой панели; `DisposeAsync()` панели не гасит shared dispatcher; `HandProgram` write-path’ы блокируются во время runtime/terminal window.
...
- `IoEditorDialog`, `AiCallCheck`, `PidRegulatorCheck`, `RtdCalCheck` считаются соседними manual write risks и должны быть явно зафиксированы как residual/follow-up.
...
Manual ConnectionTest открыт во время runtime: backend deny должен сработать до входа в manual session и до старта dispatcher.
ConnectionTest уже открыт, а runtime потом стал active: дальнейшие read/write/mode actions должны быть запрещены.
Закрытие ConnectionTest: не должно гасить shared ModbusDispatcher, если им уже владеет runtime.
HandProgram: backend write-path’ы должны блокироваться не только на уровне открытия окна, но и внутри самих действий.
...
Сценарий	Сейчас / риск	После пакета	Ключевые места
Открыт ConnectionTest во время live runtime	Панель может войти в manual session и запустить shared dispatcher	Backend deny до EnterConnectionTest() и до StartAsync()	ConnectionTestPanel.razor, DiagnosticManualSessionState.cs
Панель уже открыта, runtime потом стал active	Открытая панель всё ещё может читать/писать	Read/write/mode actions блокируются runtime gate’ом	ConnectionTestPanel.razor
...
HandProgram уже открыт, начался runtime/reset/terminal window	UI-open gate недостаточен; backend writes всё ещё возможны	Write-path’ы блокируются внутри действий	InteractiveLampOutput.razor, GasScheme.razor, HeatingScheme.razor, HotWaterScheme.razor
Соседние manual screens	Сейчас прямые OPC writes остаются возможны	В этом пакете не меняются; фиксируются как residual risk	IoEditorDialog.razor.cs, AiCallCheck.razor.cs, PidRegulatorCheck.razor.cs, RtdCalCheck.razor.cs
...
12. Manual UI-Block Без Backend Gate Недостаточен

Инвариант:
Ручная диагностика и ручные write-path’ы не должны вмешиваться в live runtime только потому, что UI что-то скрыл.
Что обязано сохраниться:
Оператор по-прежнему может пользоваться manual tools вне active runtime/terminal/reset windows.
Что меняется:
Вводится backend gate для ConnectionTestPanel.razor и write-path’ов HandProgram.
Что вне пакета:
Полный security/permissions redesign всех инженерных экранов.
13. Shared ModbusDispatcher Нельзя Ломать Панелью ConnectionTest
...
14. HandProgram Нужно Закрывать Не Только На Входе, Но И На Действии

Инвариант:
Если manual окно уже открыто, runtime всё равно должен быть защищён от backend write.
Что обязано сохраниться:
Safe manual actions вне runtime доступны как раньше.
Что меняется:
Gate добавляется внутрь write-actions в InteractiveLampOutput.razor, GasScheme.razor, HeatingScheme.razor, HotWaterScheme.razor.
Что вне пакета:
Полное покрытие вообще всех manual screens; соседние write-surfaces остаются отдельным follow-up.
15. CurrentInterrupt Consumers Изменятся Только Там, Где Это Намеренно
...
16. Docs Должны Догнать Код, А Не Наоборот
...
17. Incident/Change След Должен Быть Явным
