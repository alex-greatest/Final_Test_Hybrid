...
- Новая проверка: чтение регистра `1054` (с учётом `_settings.BaseAddressOffset`) и сравнение с рецептом `NumberOfContours` (значения `1/2`, где `1` — одноконтурный, `2` — двухконтурный, без маппинга). При несовпадении — `Fail` с заданным текстом из 3 пунктов.
...
### Бизнес-ограничение проекта (текущее)

- В текущем контуре используем только 2 варианта:
  - `1` — одноконтурный
  - `2` — двухконтурный
- Проверка `1054` выполняется прямым сравнением с `NumberOfContours` (`1==1`, `2==2`), без маппинга.
- Логику `IsDualCircuit` не меняем: dual определяется как `NumberOfContours == 2`.
...
- Рецепт `NumberOfContours` в текущем контуре содержит только:
  - `1` — одноконтурный котёл
  - `2` — двухконтурный котёл
- Проверка регистра `1054` выполняется прямым сравнением с `NumberOfContours` (`1==1`, `2==2`), без маппинга.
- Логика `IsDualCircuit` сохраняется без изменений: dual определяется как `NumberOfContours == 2`.
