...
- Для групповых операций над параметрами/результатами использовать один источник списка (single source of truth); точечные дублирующие вызовы вне общего списка считать лишним кодом и удалять.
- Метаданные параметров (например `IsRanged`, единицы, границы) определять по контракту потребителя для каждого параметра отдельно, а не копировать «по аналогии» с соседними полями.
